...
class GuiViewFx(inputHandler: InputHandler) extends Observer[GameEvent] {
  ...
  object CheckersGuiApp extends JFXApp3 {
    private lazy val moveExecutor = Executors.newCachedThreadPool() // Allows multiple concurrent task attempts
    var inputHandler: InputHandler = uninitialized
    var controller: IController = uninitialized
    var guiInstance: GuiViewFx = uninitialized
